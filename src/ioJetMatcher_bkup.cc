...
/* /1* ioJetMatcher_outlier::operator bool () { return is_set; }; *1/ */
...
/*     dict += options; */
/*     /1* cout << "name " << dict["name"].str() << "  markercolor " << dict["MarkerColor"] << endl; *1/ */
...
ioJetMatcherX::ioJetMatcherX (const char* _name, ioXsec& _Xsec, 
    ...
    match_bounds { options["match_bounds"], options["pt_true"], options["pt_measured"] },
...
void ioJetMatcherX::do_matching(int pthatbin) {
    ...
    for (auto& match : matches) {
        ...
        // tag split
        if (true) response_A->Fill(match.first, match.second,W);
    ...
    for (auto& miss : misses) {
        ...
        // tag split
        if (true) response_A->Miss(miss, W);
    ...
    /* for (auto& MC : data_MC) { */
        ...
        /* if (b_hg2_Xsec_vs_T) hg2_Xsec_vs_T->Fill(MC.pT,pthat_val); */
        /* bool found_match {false}; */
        /* for (auto& reco : data_reco) { */
            ...
            /* if (delta_R2 != 0) { */
                /* found_match = true; */
                /* reco.is_matched = true; */
                /* if (b_hg2_Xsec_vs_match) hg2_Xsec_vs_match->Fill(MC.pT,pthat_val); */
